...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:orgba276d6}).

\section{Background}
\label{sec:org1e6123d}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:orga4434cf}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org7d6002c}
...
\label{sec:org0c5490d}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org9b1d284} and are as follows:
...
The individual images in figure:\ref{fig:org810e393} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org6cbf05c} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org5bebebd}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org810e393}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{org95efcaa}.

\section{Language Choices}
\label{sec:orge2ebccb}
...
In figure:\ref{fig:orgec1003d} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:orgec1003d}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:orgc48ea10} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:orgc48ea10}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:org60e32bb}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org9de151d}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org1d72141}
...
Table:\ref{tab:orge2ca99d} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org4bb243d}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{org994b5d6}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:orga46854f}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:orga76a29f}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:orga76a29f}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:org3b1e08a}
...
\caption{\label{fig:org938091f}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org08f2041}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:orgc868e70}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:orgc868e70}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:orgaadc7d3} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:orgaadc7d3}
...
\label{sec:org2e1080a}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org084bf66} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org084bf66}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:orgaadc7d3} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:orgcb0a197}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:orgcb0a197}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org0310578}
...
Here, the pipeline presented in figure:\ref{fig:org5b99ced} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:org5b99ced}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:org5b99ced}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:org5e2d36d}
...
\label{sec:org454c9c5}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:orgcb0a197}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org5a24b74}.

\subsection{Implementation}
\label{sec:orge7ed82e}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{orgd81dd94} this new algorithm, in full, is shown.
...
\label{sec:org11b9ee2}

Figure:\ref{fig:org40b9700} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:org40b9700}
...
\label{sec:org8ad78c4}

Figure:\ref{fig:org31ab89d} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:org31ab89d}
...
\end{minted}
\caption{\label{org746407e}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org7c1d922}
...
\end{minted}
\caption{\label{org53d7f1c}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:orgf3626db}
...
\end{minted}
\caption{\label{org80a5201}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:org3fe904b}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:orgf8d7fd6} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:orgf8d7fd6}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org09745e5} provides the python model implemented.
...
\caption{\label{fig:org0221876}
The Initial Data Loading Window for the GUI}
\end{figure}
\subsubsection{Table View}
\label{sec:org5f92d17}
...
\label{sec:org37173bd}
In example:\ref{org2f90cdd} lines 11,12,13 show how the UI object has individual members, each of these has a "clicked" trigger, which when triggered calls a function pointer which this controller handles.
\begin{listing}[htbp]
\begin{minted}[]{python}
...
One example of this is in the load data function shown in example:\ref{org7c6d622}. Here the custom exception classes are imported and used for data already loaded, as well as the loading functions itself.
...
To do this, the existing canvas window is repainted based on parameter input. Example:\ref{org8c3c4f7}
...
An example of this is shown when dividing data by ploidy and comparing distributions via histograms in figure:\ref{fig:org1ba05ae}.

\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/facet_example.png}
\caption{\label{fig:org1ba05ae}
...
Example:\ref{org7f2b408} shows on lines 3,5,7 how decisions are made to best arrange the grid. Decisions are made based on the unique number of columns a grouping variable provides. As well this,  additional arguments are  used to indicate further aesthetics of the plots.
...
def facet_hist(self, df, column, group_by):
    ...
    return g.fig
\end{minted}
\caption{\label{org7f2b408}
Using Facet wrapping to provide}
\end{listing}

\section{Data Analysis Methods}
\label{sec:org852823f}
...
\label{sec:orgeee0cc0}

Modern Bayesian interpretations use the simplified model of Bayes original theorem, equation:\ref{eq:orgd244a9d} is the basis of the implemented model in this project. This states that: the posterior is proportional to the likelihood times the prior.
...
\caption{\label{fig:org5ef9270}
Normal\((\bar{x},2s)\)}
\end{figure}


\subsubsection{\(\sigma\) (Standard Deviation)}
\label{sec:orgce3c27d}
...
Figure:\ref{fig:org6e8d6bc} provides the simulated data in the form of a histogram where two groups, 'g1' and 'g2' have been used. These are considered the input for the model priors.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/exampledata.png}
\caption{\label{fig:org6e8d6bc}
Histogram of example input data}
\end{figure}

\subsubsection{Posterior Output}
\label{sec:org0e25fb9}
By applying the method outlined in code example:\ref{org09745e5} and defined by equation:\ref{eq:org01ad11c} the posterior predictions can generate 1000 random samples from the perceived distributions of the data. Based on the probability of each value existing within the priors' distribution.

Figure:\ref{fig:orgf2b213d} demonstrates the expanded data set created by the posterior predictions.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes.png}
\caption{\label{fig:orgf2b213d}
Histogram of example posterior data}
\end{figure}

\subsubsection{Difference of Means}
\label{sec:org0dbfc62}
...
For the provided example data this model states that there is a 25.3\% chance that the two means of these groups are different. This is represented in figure:\ref{fig:orgf4e9175}.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes2.png}
\caption{\label{fig:orgf4e9175}
...
\section{Morphometric Features}
\label{sec:org339a181}
...
Values used in statistical functions and measurements are presented as metric units, derived from \textmu{}-CT image pixel values. The equation:\ref{eq:org6e32f71} is presented here.
...
\section{Length of Grains is Not Effected by Domestication}
\label{sec:orgda20b14}
...
\begin{figure}[!ht]
  \subfloat[Difference of means between \textit{T. monococcum} and \textit{T. beoticum} Length  \label{subfig-1:length}]{%
    \includegraphics[width=0.45\textwidth]{./images/results/group1/bayes_difference_of_means_length.png}
  }
  \hfill
  \subfloat[Difference of means between \textit{T. dicoccum} and \textit{T. dicoccoides} Width  \label{subfig-2:Width}]{%
...
\subsection{Tools}
\label{sec:org09003e7}
\begin{table}[htbp]
\caption{\label{tab:orgb66b35b}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\clearpage
\chapter{Glossary}
\label{sec:org1d2a7d0}
\begin{table}[htbp]
\caption{\label{tab:orgba276d6}
...
\label{sec:org45e018e}
\begin{table}[htbp]
\caption{\label{tab:orga4434cf}
...
\label{sec:orge874d22}

\section{Setup.py}
\label{sec:org4aecc22}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1]{python}
...
\label{sec:org720f1e0}

\subsection{Data Distributions}
\label{sec:org1b54a2d}
...
\label{sec:orgc5d49cb}
\label{org95efcaa}

\subsection{Sprint - Week 0}
\label{sec:org160c4be}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{org8221e0f}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
