...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:orgc607e22}).

\section{Background}
\label{sec:org65e53a2}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:org248c1b4}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org23f8fab}
...
\label{sec:org6a05115}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org1698b29} and are as follows:
...
The individual images in figure:\ref{fig:orgdfbee77} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:orgab74d9a} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org0f1eacf}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:orgdfbee77}.
...
A key function of the CT Analysis Library was created, showing in listing:\ref{org8ac09cf}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
...
In figure:\ref{fig:orgf3d3778} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:orgf3d3778}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org6c64bcd} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org6c64bcd}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:orgadf503c}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org8e80769}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org9f1a7be}
...
Table:\ref{tab:org396c5db} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org6d20bc3}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{orgb976c26}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org5505a58}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org8c046d8}
\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/github.png}
\caption{\label{fig:org8c046d8}
Github Issue Tracking}
\end{figure}

\section{Implementation}
\label{sec:org1b351cc}
...
\caption{\label{fig:org4efef61}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:orgd03079d}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:orgcf35783}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:orgcf35783}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org70f8f38} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org70f8f38}
...
\label{sec:orgb32cbaa}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org367d756} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org367d756}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org70f8f38} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:orgb36a5c2}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:orgb36a5c2}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org184f838}
...
Here, the pipeline presented in figure:\ref{fig:orgc85ee1a} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orgc85ee1a}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:orgc85ee1a}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:orgca95666}
...
\label{sec:org42ecf20}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:orgb36a5c2}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org07fa39f}.

\subsubsection{Implementation}
\label{sec:org5a57d52}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{org2c12415} this new algorithm, in full, is shown.
...
\label{sec:orgf4e1689}

Figure:\ref{fig:org97ec49f} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:org97ec49f}
...
\label{sec:org841f492}

Figure:\ref{fig:org7469682} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:org7469682}
...
\end{minted}
\caption{\label{org74fd38f}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org4973877}
...
\end{minted}
\caption{\label{orgeca15e2}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:org8efb6f2}
...
\label{sec:org347ee4f}

\section{CT GUI Application Methods}
\label{sec:orga3a5ae8}
\subsection{Integration with CT Analysis Library}
\label{sec:org8901634}
\subsection{QT GUI Loading}
\label{sec:orgc35a85b}
\subsection{Connecting Signals from GUI to Function Calls}
\label{sec:org1a865ae}
\subsection{Plot Window Rendering}
\label{sec:org2a31fb0}
\subsection{Creating GUI Elements When Required}
\label{sec:org2636113}
\subsection{Dynamically Generating Plots}
\label{sec:orgb865f8f}
\section{Data Analysis Methods}
\label{sec:orgf59d00c}
\section{Bayesian Modelling}
\label{sec:org9f61283}

\chapter{Results}
\label{sec:org5426fd0}
\chapter{Discussion}
\label{sec:org1044377}
\section{Similar Research}
\label{sec:orge562a79}
\section{Alternate Solutions}
\label{sec:orga93b09e}
\chapter{Critical Evaluation}
\label{sec:org95a3348}
\section{Organisational Methods}
\label{sec:orgdb55ec7}
\section{Relevance to Degree}
\label{sec:orgd0cda3b}
\section{Time Management}
\label{sec:orgccd15d2}
\section{Collaborative Work}
\label{sec:org3c61103}
\section{Other Issues}
\label{sec:orgc70bec1}
\chapter{Appendix}
\label{sec:orge65c1a1}
\section{\emph{Software Packages Used}}
\label{sec:orgc7c8cf0}

\subsection{Libraries}
\label{sec:orgffe0b2e}
\begin{table}[htbp]
\caption{\label{tab:org396c5db}
...
\subsection{Tools}
\label{sec:orgf8f679d}
\begin{table}[htbp]
\caption{\label{tab:org3622740}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:orgba38bc9}
\begin{table}[htbp]
\caption{\label{tab:orgc607e22}
...
\label{sec:org28e741e}
\begin{table}[htbp]
\caption{\label{tab:org248c1b4}
