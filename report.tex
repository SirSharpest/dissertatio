...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:org50f78bd}).

\section{Background}
\label{sec:org06eebdf}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:org367fef6}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org4f4c287}
...
\label{sec:org0837906}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org0101446} and are as follows:
...
The individual images in figure:\ref{fig:org1a93660} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org98b1033} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org0becf86}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org1a93660}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{org42e0f12}.

\section{Language Choices}
\label{sec:orgf5451f1}
...
In figure:\ref{fig:orgaaef81d} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:orgaaef81d}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:orge9d7c07} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:orge9d7c07}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:orgc722d30}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{orge290638}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org41bf057}
...
Table:\ref{tab:orgf26e190} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org613c16c}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{orgcb85988}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org400df1b}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org7af2c48}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:org7af2c48}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:orge47fddb}
...
\caption{\label{fig:orgd1271fc}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:orgedc2f08}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:org0e49671}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:org0e49671}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org9012f8a} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org9012f8a}
...
\label{sec:org1d5a800}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org24c6099} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org24c6099}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org9012f8a} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:org79a09f5}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:org79a09f5}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org7fde74f}
...
Here, the pipeline presented in figure:\ref{fig:org422aef5} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:org422aef5}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:org422aef5}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:org6b9256c}
...
\label{sec:org5fec949}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:org79a09f5}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org8727b8a}.

\subsection{Implementation}
\label{sec:org0ecd482}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{org04965a1} this new algorithm, in full, is shown.
...
\label{sec:org0636695}

Figure:\ref{fig:org1e18a2a} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:org1e18a2a}
...
\label{sec:orgb01392d}

Figure:\ref{fig:orge94c9b9} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:orge94c9b9}
...
\end{minted}
\caption{\label{orgdc138ea}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:orga4ed8dc}
...
\end{minted}
\caption{\label{org6f9d3fd}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:orgb9b5fd6}
...
\end{minted}
\caption{\label{org646ca52}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:org0ae367a}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:orgda8cb27} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:orgda8cb27}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org6d36a0f} provides the python model implemented.
...
\label{sec:org9ed0429}
\subsection{Connecting Signals from GUI to Function Calls}
\label{sec:org7a15da9}
\subsection{Plot Window Rendering}
\label{sec:orga957beb}
\subsection{Creating GUI Elements When Required}
\label{sec:orge8c14a4}
\subsection{Dynamically Generating Plots}
\label{sec:org6d5326e}
\section{Data Analysis Methods}
\label{sec:org91ebe75}
\section{Bayesian Modelling}
\label{sec:orgbc63fe0}

\chapter{Results}
\label{sec:orge1402ca}
\chapter{Discussion}
\label{sec:orgf04ad2f}
\section{Similar Research}
\label{sec:org19a2bca}
\section{Alternate Solutions}
\label{sec:orge728a7d}
\chapter{Critical Evaluation}
\label{sec:org8f3a57f}
\section{Organisational Methods}
\label{sec:org771098e}
\section{Relevance to Degree}
\label{sec:orge5f7a12}
\section{Time Management}
\label{sec:org8da4312}
\section{Collaborative Work}
\label{sec:orgcad47d8}
\section{Other Issues}
\label{sec:org2258a03}


\appendix
\chapter{\emph{Software Packages Used}}
\label{sec:org83887ff}
\subsection{Libraries}
\label{sec:orgfbaeb2d}
\begin{table}[htbp]
\caption{\label{tab:orgf26e190}
...
\subsection{Tools}
\label{sec:orgf779482}
\begin{table}[htbp]
\caption{\label{tab:org62e8e86}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:orga2b8bc2}
\begin{table}[htbp]
\caption{\label{tab:org50f78bd}
...
\label{sec:orgc6206b1}
\begin{table}[htbp]
\caption{\label{tab:org367fef6}
...
\label{sec:org5197eed}

\section{Setup.py}
\label{sec:org2281c6c}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true]{python}
...
\label{sec:orgccd5d85}
\label{org42e0f12}

\subsection{Sprint - Week 0}
\label{sec:org0c5dbb4}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{org6b6717c}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
