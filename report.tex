...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:org486ca32}).

\section{Background}
\label{sec:org6048fe3}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:org2675f82}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:orgaf0659b}
...
\label{sec:org7cb28d2}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:orge4f166e} and are as follows:
...
The individual images in figure:\ref{fig:orge861d23} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:orgc12ee23} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org1279d13}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:orge861d23}.
...
A key function of the CT Analysis Library was created, showing in listing:\ref{org7a8c362}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
...
In figure:\ref{fig:org985845e} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org985845e}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org907af37} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org907af37}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:org388055f}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org2582e46}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org1a3bb37}
...
Table:\ref{tab:org99a7a41} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org2fb54e4}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{org5ebac2e}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org49c355e}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org0ab73bf}
\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/github.png}
\caption{\label{fig:org0ab73bf}
Github Issue Tracking}
\end{figure}

\section{Implementation}
\label{sec:org97c026c}
...
\caption{\label{fig:org189fe66}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:orge57c310}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:org69f4d19}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:org69f4d19}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org6914134} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org6914134}
...
\label{sec:org84803be}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:orgde9f098} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:orgde9f098}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org6914134} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:orga86f3ad}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:orga86f3ad}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org3263b4c}
...
Here, the pipeline presented in figure:\ref{fig:orga46c52c} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orga46c52c}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:orga46c52c}
How data is integrated with the CT Analysing Library}
\end{figure}


\section{Image Analysis Methods}
\label{sec:org9c8d961}
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:orga4e6b2c}.
\subsubsection{Effect of Enhanced Watershed algorithm}
\label{sec:org0c5711c}

Figure:\ref{fig:orgaa4503b} shows in two dimensional flat images how this problem was solved.
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/chess_quasi.png}
\caption{\label{fig:orgaa4503b}
\emph{A} showing the chessboard method, \emph{B} improved quasi-euclidean method}
\end{figure}

Figure:\ref{fig:org8fba467} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.


\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/watershedfigure.png}
\caption{\label{fig:org8fba467}
\emph{A} showing before the new method, \emph{B} after, \emph{C} zoomed before, \emph{D} zoomed after.}
\end{figure}

\subsection{Extracted Grains}
\label{sec:orgb704135}

\begin{figure}[htbp]
\centering
\includegraphics[width=13cm]{./images/ctgrains.png}
\caption{\label{fig:org312f323}
Individual Wheat grains, rendered in 3D}
\end{figure}

\section{CT Analysing Library Methods}
\label{sec:orga30baaf}
\subsection{Spike Rejoining}
\label{sec:orgceac4ec}
\subsection{Data Aggregating}
\label{sec:org3edf99d}
\subsection{Loading Data}
\label{sec:org2fa1fd6}
\subsection{Cleaning and Removing Erroneous Data}
\label{sec:org864d92c}
\subsection{Data Transformations}
\label{sec:org73484b7}
\subsection{T-Tests}
\label{sec:org446bc48}
\subsection{Bayesian Hypothesis Testing}
\label{sec:org062d2ce}
\subsection{Plotting Functions}
\label{sec:org230fa2a}
\subsection{Adding Spike Experiment Information}
\label{sec:org580724c}
\begin{listing}[htbp]
\begin{minted}[]{python}
...
\subsection{Tools}
\label{sec:orgc5d3422}
\begin{table}[htbp]
\caption{\label{tab:org1c3110f}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:orgccabb4c}
\begin{table}[htbp]
\caption{\label{tab:org486ca32}
...
\label{sec:org6271758}
\begin{table}[htbp]
\caption{\label{tab:org2675f82}
...
\label{sec:orgf1dbcb2}
\subsection{MATLAB Watershedding}
\label{sec:orgf5f27d0}
