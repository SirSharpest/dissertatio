...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:orgaa1ab98}).

\section{Background}
\label{sec:org2f311ee}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:orgcddc0c4}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:orgbf6b1c1}
...
\label{sec:org99bb757}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org16901d0} and are as follows:
...
The individual images in figure:\ref{fig:org2a6b119} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org0b20afb} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org0215119}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org2a6b119}.
...
A key function of the CT Analysis Library was created, showing in listing:\ref{orgb355be4}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
...
In figure:\ref{fig:org9a4d354} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org9a4d354}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org89517b2} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org89517b2}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:org1e0ecfe}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org1582740}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{orgef91864}
...
Table:\ref{tab:orgb2f3e12} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:orgf4a45c5}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{orga333941}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org0178955}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org6e74f46}
\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/github.png}
\caption{\label{fig:org6e74f46}
Github Issue Tracking}
\end{figure}

\section{Implementation}
\label{sec:org92e4ca3}
...
\caption{\label{fig:org25ad240}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:orge9fea85}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:org316dc51}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:org316dc51}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:orga4c9bd3} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:orga4c9bd3}
...
\label{sec:org769c36c}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org6a160c4} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org6a160c4}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:orga4c9bd3} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:org6c2e780}

\begin{figure}[htbp]
\centering
\includegraphics[width=13cm]{./images/matlab.png}
\caption{\label{fig:org6c2e780}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:orgca0ecba}
...
Here, the pipeline presented in figure:\ref{fig:org44416d2} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
\caption{\label{fig:org44416d2}
How data is integrated with the CT Analysing Library}
\end{figure}


\section{Image Analysis Methods}
\label{sec:orga9d2f9a}
Initially, this project aimed to use existing software for feature extraction

\subsection{New Watershed Algorithm}
\label{sec:orge1191d5}
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:orgc93cd60}.
\subsubsection{Effect of Enhanced Watershed algorithm}
\label{sec:orgacbd94e}

Figure:\ref{fig:org2b76b6c} shows in two dimensional flat images how this problem was solved.
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/chess_quasi.png}
\caption{\label{fig:org2b76b6c}
\emph{A} showing the chessboard method, \emph{B} improved quasi-euclidean method}
\end{figure}

Figure:\ref{org03424a5} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.


\#+ATTR\_LATEX :width 10cm
\begin{center}
\includegraphics[width=.9\linewidth]{./images/watershedfigure.png}
\label{org03424a5}
\end{center}

\subsection{Extracted Grains}
\label{sec:org0cee772}

\begin{figure}[htbp]
\centering
\includegraphics[width=13cm]{./images/ctgrains.png}
\caption{\label{fig:orgfe7f053}
Individual Wheat grains, rendered in 3D}
\end{figure}

\section{CT Analysing Library Methods}
\label{sec:org948918e}
\subsection{Spike Rejoining}
\label{sec:org1480eba}
\subsection{Data Aggregating}
\label{sec:org454c279}
\subsection{Loading Data}
\label{sec:orgcdc9621}
\subsection{Cleaning and Removing Erroneous Data}
\label{sec:org50b1f2e}
\subsection{Data Transformations}
\label{sec:org4f513c6}
\subsection{T-Tests}
\label{sec:orgdd69723}
\subsection{Bayesian Hypothesis Testing}
\label{sec:orgb4ee6ca}
\subsection{Plotting Functions}
\label{sec:org1db02a3}
\subsection{Adding Spike Experiment Information}
\label{sec:orgcecbb6f}
\begin{listing}[htbp]
\begin{minted}[]{python}
...
\subsection{Tools}
\label{sec:org3520d87}
\begin{table}[htbp]
\caption{\label{tab:orgb5f0f12}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:org78fad0f}
\begin{table}[htbp]
\caption{\label{tab:orgaa1ab98}
...
\label{sec:orgca2804c}
\begin{table}[htbp]
\caption{\label{tab:orgcddc0c4}
...
\label{sec:orgd6264fb}
\subsection{MATLAB Watershedding}
\label{sec:orgf246760}
