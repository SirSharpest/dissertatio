...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the glossary (table:\ref{tab:org45be433}).

\section{Background}
\label{sec:orgf636d98}
...
\end{multicols}
Full species names are found in table:\ref{tab:org76f2ccd}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:orgdb57217}
...
\label{sec:org05492f2}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org5e14bde} and are as follows:
...
The individual images in figure:\ref{fig:org32b7000} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:orgf035a78} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org4fefa88}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org32b7000}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{org94e3d03}.

\section{Language Choices}
\label{sec:org484c4c8}
...
In figure:\ref{fig:org1b3414a} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org1b3414a}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:orgf067343} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:orgf067343}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:orga30ba65}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{orge9fec74}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{orge978917}
...
Table:\ref{tab:org5dfae21} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:orgd9f6f8f}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{org293948b}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:orgac6c1b8}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:orgc3751b6}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:orgc3751b6}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:org74f65e6}
...
\caption{\label{fig:org97b5160}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org4cd8b82}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:org6f9e10c}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:org6f9e10c}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org0a2adbf} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org0a2adbf}
...
\label{sec:org297960b}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org0d832ad} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org0d832ad}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org0a2adbf} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:org7e340a7}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:org7e340a7}
Image Processing Pipeline}
\end{figure}

\subsection{Morphometric Features}
\label{sec:org9ae745a}
...
Values used in statistical functions and measurements are presented as metric units, derived from \textmu{}-CT image pixel values. The equation:\ref{eq:org58e2be7} is presented here.
...
Here, the pipeline presented in figure:\ref{fig:orgc9ecc83} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
An additional phenotype is created to describe the interaction between the geometric parameters; the interaction is described in equation:\ref{eq:org6e98cea}.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orgc9ecc83}.
\clearpage
\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/pipeline.png}
\caption{\label{fig:orgc9ecc83}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:org0ee3a4a}
...
\label{sec:org8db9d1c}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:org7e340a7}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org6e1ee4e}.

\subsection{Implementation}
\label{sec:orgd47c279}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{org593e4c8} this new algorithm, in full, is shown.
...
\label{sec:org6b5120d}

Figure:\ref{fig:orgec1f96d} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:orgec1f96d}
...
\label{sec:orgb2dd864}

Figure:\ref{fig:orgeba2b7b} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/watershedfigure.png}
\caption{\label{fig:orgeba2b7b}
...
\end{minted}
\caption{\label{orgd1f5abd}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org7d9d6d8}
...
\end{minted}
\caption{\label{org7199ab7}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:org4808b26}
...
\end{minted}
\caption{\label{org1c379b8}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:org76fb33f}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:orgf9ba5dc} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:orgf9ba5dc}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org16b685f} provides the python model implemented.
...
\caption{\label{fig:org7b8c1dd}
The Initial Data Loading Window for the GUI}
\end{figure}
\subsubsection{Table View}
\label{sec:org79bec96}
...
\label{sec:orgee28bd0}
In example:\ref{orga248df0} lines 11,12,13 show how the UI object has individual members, each of these has a "clicked" trigger, which when triggered calls a function pointer which this controller handles.
\begin{listing}[htbp]
...
One example of this is in the load data function shown in example:\ref{org1e49c9e}. Here the custom exception classes are imported and used for data already loaded, as well as the loading functions itself.
...
To do this, the existing canvas window is repainted based on parameter input. Example:\ref{org40edc09}
...
An example of this is shown when dividing data by ploidy and comparing distributions via histograms in figure:\ref{fig:org6e95006}.

\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/facet_example.png}
\caption{\label{fig:org6e95006}
...
Example:\ref{org85be864} shows on lines 3,5,7 how decisions are made to best arrange the grid. Decisions are made based on the unique number of columns a grouping variable provides. As well this,  additional arguments are  used to indicate further aesthetics of the plots.
...
def facet_hist(self, df, column, group_by):
    ...
    return g.fig
\end{minted}
\caption{\label{org85be864}
Using Facet wrapping to provide}
\end{listing}

\section{Data Analysis Methods}
\label{sec:org691011b}
...
\label{sec:org194c7d4}

Modern Bayesian interpretations use the simplified model of Bayes original theorem, equation:\ref{eq:orgcf21470} is the basis of the implemented model in this project. This states that: the posterior is proportional to the likelihood times the prior.
...
\caption{\label{fig:org51941db}
Normal\((\bar{x},2s)\)}
\end{figure}


\subsubsection{\(\sigma\) (Standard Deviation)}
\label{sec:org01edc0f}
...
Figure:\ref{fig:orgf7bebbb} provides the simulated data in the form of a histogram where two groups, 'g1' and 'g2' have been used. These are considered the input for the model priors.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/exampledata.png}
\caption{\label{fig:orgf7bebbb}
Histogram of example input data}
\end{figure}

\subsubsection{Posterior Output}
\label{sec:org358487e}
By applying the method outlined in code example:\ref{org16b685f} and defined by equation:\ref{eq:org4995280} the posterior predictions can generate 1000 random samples from the perceived distributions of the data. Based on the probability of each value existing within the priors' distribution.

Figure:\ref{fig:org490a1c0} demonstrates the expanded data set created by the posterior predictions.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes.png}
\caption{\label{fig:org490a1c0}
Histogram of example posterior data}
\end{figure}

\subsubsection{Difference of Means}
\label{sec:org6f6bf11}
...
For the provided example data this model states that there is a 25.3\% chance that the two means of these groups are different. This is represented in figure:\ref{fig:org7c6ea31}.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes2.png}
\caption{\label{fig:org7c6ea31}
...
Hexaploids (\emph{T. aestivum}) when compared to tetraploids (\emph{T. dicoccum}) reports a 86.2\% chance of the two means being the same. Indicating that increasing of ploidy fron 4N to 6N does not produce a significant change in grain volume. Figure:\ref{fig:org1174950} shows this, with full results shown in supplemental figure:\ref{fig:group6}.

\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/results/ploidyvol.png}
\caption{\label{fig:org1174950}
...
A wide range of attributes which previous studies could not access \cite{Gegas2010} is made available for study. In figure:\ref{fig:org11f738c} the available phenotypes are displayed in a pairwise fashion, demonstrating how interesting relationships between attributes could provide more information if studied further.
...
With this data, similar solutions could be implemented. Figure:\ref{fig:orgd5e81cd} shows all grains in this study, grouped by origin spike and plotted by a two component PCA. Input is calculated through averaging across spike grains' length, width, depth, volume and the geometric interaction term, via equation:\ref{eq:orgc9ec66c}.
...
\caption{\label{fig:orgd5e81cd}
A 2 component PCA showing all the spikes discussed in this study}
\end{figure}


\chapter{Critical Evaluation}
\label{sec:orgbcb9639}
\section{Organisational Methods}
\label{sec:org3bb89df}
\section{Relevance to Degree}
\label{sec:orge5cfbc1}
\section{Time Management}
\label{sec:org01591ec}
\section{Collaborative Work}
\label{sec:org5db2bdd}
\section{Other Issues}
\label{sec:org5342e0b}


\appendix

\chapter{Glossary}
\label{sec:org88ef050}
\begin{table}[htbp]
\caption{\label{tab:org45be433}
...
\label{sec:org2db100a}

\subsection{Libraries}
\label{sec:org1ac9207}
\begin{table}[htbp]
\caption{\label{tab:org5dfae21}
...
\subsection{Tools}
\label{sec:org9e6997b}
\begin{table}[htbp]
\caption{\label{tab:org14d180a}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\section{Documentation}
\label{sec:org7806f57}
\clearpage

\chapter{Wheat Varieties}
\label{sec:orgceb255d}
\begin{table}[htbp]
\caption{\label{tab:org76f2ccd}
...
\label{sec:org1aa04e5}

\section{Setup.py}
\label{sec:org3fa269c}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true,fontfamily=DejaVuSans]{python}
...
\subsection{Pair Plot}
\label{sec:org787fe9c}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group2/pairplot.png}
\label{fig:orgc99a4a2}
\end{figure}

\clearpage
\subsection{PCA}
\label{sec:orga33febb}
\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group2/pca.png}
\label{fig:orgdbfbddf}
\end{figure}
...
\label{sec:org175c421}

\subsection{Data Distributions}
\label{sec:orgafde5fb}
...
\label{sec:org5ae72ce}

\subsection{Data Distributions}
\label{sec:org1874b5f}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{org0667a4f}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
