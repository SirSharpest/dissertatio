...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:orgf3aa610}).

\section{Background}
\label{sec:org959dd72}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:orga3b6848}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org3079a76}
...
\label{sec:orgfaa52da}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:orga776fd8} and are as follows:
...
The individual images in figure:\ref{fig:orgaccec92} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org1134395} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org3ee3709}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:orgaccec92}.
...
A key function of the CT Analysis Library was created, showing in listing:\ref{org2836485}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
...
In figure:\ref{fig:orga33a600} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:orga33a600}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org7eb4310} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org7eb4310}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:orgb5329cc}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org418fe7c}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org2a2b510}
...
Table:\ref{tab:org83f72d8} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:orgad918ad}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{orga8aa908}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org782c959}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org77aee72}
\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/github.png}
\caption{\label{fig:org77aee72}
Github Issue Tracking}
\end{figure}

\section{Implementation}
\label{sec:org2727088}
...
\caption{\label{fig:org85fa389}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org4a932bb}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:org1ceaff6}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:org1ceaff6}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:orgee18d8c} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:orgee18d8c}
...
\label{sec:orgef1754f}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org158f371} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org158f371}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:orgee18d8c} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:org255cafa}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:org255cafa}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org58c608f}
...
Here, the pipeline presented in figure:\ref{fig:orga8f318d} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orga8f318d}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:orga8f318d}
How data is integrated with the CT Analysing Library}
\end{figure}


\section{Image Analysis Methods}
\label{sec:orgbef6553}
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org8b48c98}.
\subsubsection{Effect of Enhanced Watershed algorithm}
\label{sec:org689bdd7}

Figure:\ref{fig:org278d67f} shows in two dimensional flat images how this problem was solved.
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/chess_quasi.png}
\caption{\label{fig:org278d67f}
\emph{A} showing the chessboard method, \emph{B} improved quasi-euclidean method}
\end{figure}

Figure:\ref{fig:orga5e2d96} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.


\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/watershedfigure.png}
\caption{\label{fig:orga5e2d96}
\emph{A} showing before the new method, \emph{B} after, \emph{C} zoomed before, \emph{D} zoomed after.}
\end{figure}

\subsection{Extracted Grains}
\label{sec:org00b8f16}

\begin{figure}[htbp]
\centering
\includegraphics[width=13cm]{./images/ctgrains.png}
\caption{\label{fig:org0ddbd91}
Individual Wheat grains, rendered in 3D}
\end{figure}

\section{CT Analysing Library Methods}
\label{sec:orgccd3e34}
\subsection{Spike Rejoining}
\label{sec:orgfce4030}
\subsection{Data Aggregating}
\label{sec:orgfd10ff4}
\subsection{Loading Data}
\label{sec:org0064b8d}
\subsection{Cleaning and Removing Erroneous Data}
\label{sec:org8523246}
\subsection{Data Transformations}
\label{sec:org5b79f6a}
\subsection{T-Tests}
\label{sec:orgccaab3a}
\subsection{Bayesian Hypothesis Testing}
\label{sec:orgd91c782}
\subsection{Plotting Functions}
\label{sec:orgefc2188}
\subsection{Adding Spike Experiment Information}
\label{sec:orgd559ed3}
\begin{listing}[htbp]
\begin{minted}[]{python}
...
\subsection{Tools}
\label{sec:orgb0186d2}
\begin{table}[htbp]
\caption{\label{tab:org4288634}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:org2c0d2c9}
\begin{table}[htbp]
\caption{\label{tab:orgf3aa610}
...
\label{sec:org64e70a0}
\begin{table}[htbp]
\caption{\label{tab:orga3b6848}
...
\label{sec:org1fffc39}
\subsection{MATLAB Watershedding}
\label{sec:org13336c2}
