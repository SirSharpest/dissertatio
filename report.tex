...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:org122cb52}).

\section{Background}
\label{sec:orgd139cec}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:org7622966}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org65c8e72}
...
\label{sec:orge946a16}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org9461353} and are as follows:
...
The individual images in figure:\ref{fig:org6462bd3} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org05db82e} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org25dabc1}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org6462bd3}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{org03a1b00}.

\section{Language Choices}
\label{sec:org9da055a}
...
In figure:\ref{fig:org4437d33} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org4437d33}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org21acde9} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org21acde9}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:org78fc925}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org1f75bee}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org1a557bd}
...
Table:\ref{tab:orge1a0250} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org4d84e5c}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{orgc8b4876}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:orgf7f814a}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org274c3a1}
\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/github.png}
\caption{\label{fig:org274c3a1}
Github Issue Tracking}
\end{figure}

\section{Implementation}
\label{sec:org9603872}
...
\caption{\label{fig:org7276127}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org9877f55}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:orga5eb584}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:orga5eb584}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org22cf27d} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org22cf27d}
...
\label{sec:orgb58d926}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org49ec711} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org49ec711}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org22cf27d} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:org2efc26d}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:org2efc26d}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org740001e}
...
Here, the pipeline presented in figure:\ref{fig:org4afaa52} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:org4afaa52}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:org4afaa52}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:orge0a3f5d}
...
\label{sec:orgaaf2f2d}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:org2efc26d}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org57a744a}.

\subsection{Implementation}
\label{sec:org4d0fc32}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{orgc88228f} this new algorithm, in full, is shown.
...
\label{sec:org6a55fea}

Figure:\ref{fig:org112d293} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:org112d293}
...
\label{sec:org42434ac}

Figure:\ref{fig:orge1c820f} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:orge1c820f}
...
\end{minted}
\caption{\label{org4c4303b}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:orgd1b5c1a}
...
\end{minted}
\caption{\label{orgc5e73c6}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:org9331497}
...
\end{minted}
\caption{\label{org9a32ca7}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:org4e9b264}
...
\label{sec:orgf6dafae}
\subsection{Connecting Signals from GUI to Function Calls}
\label{sec:org695d6a1}
\subsection{Plot Window Rendering}
\label{sec:orge3e2338}
\subsection{Creating GUI Elements When Required}
\label{sec:org4be4235}
\subsection{Dynamically Generating Plots}
\label{sec:org9918270}
\section{Data Analysis Methods}
\label{sec:orgd308e2e}
\section{Bayesian Modelling}
\label{sec:orgad1d82b}

\chapter{Results}
\label{sec:orga5bc056}
\chapter{Discussion}
\label{sec:org13a8999}
\section{Similar Research}
\label{sec:orgb3f3cf6}
\section{Alternate Solutions}
\label{sec:orge7633f5}
\chapter{Critical Evaluation}
\label{sec:org0baa6ab}
\section{Organisational Methods}
\label{sec:orgb23b8c3}
\section{Relevance to Degree}
\label{sec:org2268522}
\section{Time Management}
\label{sec:orgdc1aec5}
\section{Collaborative Work}
\label{sec:org772f2f8}
\section{Other Issues}
\label{sec:org8e33293}


\appendix
\chapter{\emph{Software Packages Used}}
\label{sec:orge70b4e8}
\subsection{Libraries}
\label{sec:orgd1cbd58}
\begin{table}[htbp]
\caption{\label{tab:orge1a0250}
...
\subsection{Tools}
\label{sec:org84fbeff}
\begin{table}[htbp]
\caption{\label{tab:org1bcf58b}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:org74a4a03}
\begin{table}[htbp]
\caption{\label{tab:org122cb52}
...
\label{sec:orge862aa0}
\begin{table}[htbp]
\caption{\label{tab:org7622966}
...
\label{sec:orgbcc64d5}

\section{Setup.py}
\label{sec:org3c31e4f}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true,fontfamily=dejavu]{python}
...
\label{sec:orgf7398f1}
\label{org03a1b00}

\subsection{Sprint - Week 0}
\label{sec:org9a610e2}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{orgde5427c}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
