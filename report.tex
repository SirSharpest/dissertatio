...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:org1a013c6}).

\section{Background}
\label{sec:org1991c5d}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:org0793ed6}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:orga3ba987}
...
\label{sec:org1a6d55f}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org9f8bafd} and are as follows:
...
The individual images in figure:\ref{fig:org164a2a5} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:orgbb77fc6} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org94fe414}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org164a2a5}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{org4345703}.

\section{Language Choices}
\label{sec:orgc1325c3}
...
In figure:\ref{fig:org1ca3cb8} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org1ca3cb8}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org2915553} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org2915553}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:org2408521}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org854815f}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org0cf2c74}
...
Table:\ref{tab:org63dc111} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org13de9bd}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{org45bdbc1}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org0c35190}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:orgab9ed36}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:orgab9ed36}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:org4fcf343}
...
\caption{\label{fig:orgf2337e4}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org3f38c19}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:orgb0816dc}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:orgb0816dc}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org1e6c2ac} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org1e6c2ac}
...
\label{sec:orgfea1df0}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:orgd736b5b} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:orgd736b5b}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org1e6c2ac} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:org9e5c1c6}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:org9e5c1c6}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org841d5ee}
...
Here, the pipeline presented in figure:\ref{fig:orge12f089} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orge12f089}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:orge12f089}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:org81256c9}
...
\label{sec:orgdaf8ad1}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:org9e5c1c6}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:orgfe1ec3d}.

\subsection{Implementation}
\label{sec:orga3a72d6}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{org7bc758f} this new algorithm, in full, is shown.
...
\label{sec:orgea1fdbb}

Figure:\ref{fig:orgc25f1f1} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:orgc25f1f1}
...
\label{sec:org1f4571b}

Figure:\ref{fig:orgd084b22} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:orgd084b22}
...
\end{minted}
\caption{\label{orgfddd379}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org4c6e784}
...
\end{minted}
\caption{\label{org89756cb}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:orgcb2cb96}
...
\end{minted}
\caption{\label{org1351613}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:org6c0d27d}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:orgc11805f} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:orgc11805f}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org42f2c78} provides the python model implemented.
...
\caption{\label{fig:org84506fb}
The Initial Data Loading Window for the GUI}
\end{figure}
\subsubsection{Table View}
\label{sec:orgffb4c71}
...
\label{sec:orgff3843a}
In example:\ref{org5505c2e} lines 11,12,13 show how the UI object has individual members, each of these has a "clicked" trigger, which when triggered calls a function pointer which this controller handles.
\begin{listing}[htbp]
\begin{minted}[]{python}
...
One example of this is in the load data function shown in example:\ref{org481f05f}. Here the custom exception classes are imported and used for data already loaded, as well as the loading functions itself.
...
To do this, the existing canvas window is repainted based on parameter input. Example:\ref{orgba87d0c}
...
An example of this is shown when dividing data by ploidy and comparing distributions via histograms in figure:\ref{fig:org84dcb3f}.

\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/facet_example.png}
\caption{\label{fig:org84dcb3f}
...
Example:\ref{org5b9c15a} shows on lines 3,5,7 how decisions are made to best arrange the grid. Decisions are made based on the unique number of columns a grouping variable provides. As well this,  additional arguments are  used to indicate further aesthetics of the plots.
...
def facet_hist(self, df, column, group_by):
    ...
    return g.fig
\end{minted}
\caption{\label{org5b9c15a}
Using Facet wrapping to provide}
\end{listing}

\section{Data Analysis Methods}
\label{sec:org6299856}
...
\label{sec:org655e18d}

Modern Bayesian interpretations use the simplified model of Bayes original theorem, equation:\ref{eq:org2d1cefe} is the basis of the implemented model in this project. This states that: the posterior is proportional to the likelihood times the prior.
...
\caption{\label{fig:org6f0bb3f}
Normal\((\bar{x},2s)\)}
\end{figure}


\subsubsection{\(\sigma\) (Standard Deviation)}
\label{sec:orgd2e4307}
...
Figure:\ref{fig:orgeb790ee} provides the simulated data in the form of a histogram where two groups, 'g1' and 'g2' have been used. These are considered the input for the model priors.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/exampledata.png}
\caption{\label{fig:orgeb790ee}
Histogram of example input data}
\end{figure}

\subsubsection{Posterior Output}
\label{sec:org128b307}
By applying the method outlined in code example:\ref{org42f2c78} and defined by equation:\ref{eq:org659fbd5} the posterior predictions can generate 1000 random samples from the perceived distributions of the data. Based on the probability of each value existing within the priors' distribution.

Figure:\ref{fig:org0a02afe} demonstrates the expanded data set created by the posterior predictions.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes.png}
\caption{\label{fig:org0a02afe}
Histogram of example posterior data}
\end{figure}

\subsubsection{Difference of Means}
\label{sec:org032823f}
...
For the provided example data this model states that there is a 25.3\% chance that the two means of these groups are different. This is represented in figure:\ref{fig:org031a51b}.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes2.png}
\caption{\label{fig:org031a51b}
...
\label{sec:org5a971a8}

\subsection{Data Distributions}
\label{sec:orga21708a}
...
\label{sec:orgde98799}

\subsection{Data Distributions}
\label{sec:org55a3169}
...
\label{fig:org3d65bb8}
\end{figure}

\clearpage
\subsection{PCA}
\label{sec:org3084075}
\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group4/pca.png}
\label{fig:orgec0c0b7}
\end{figure}


\clearpage
\section{\emph{T. dicoccum} and \emph{T. durum}}
\label{sec:org602c958}

\subsection{Data Distributions}
\label{sec:orgb54d9c6}
...
\clearpage
\chapter{Discussion}
\label{sec:org9b03aad}
\section{Similar Research}
\label{sec:orge5da45a}
\section{Alternate Solutions}
\label{sec:org77a67c2}
\chapter{Critical Evaluation}
\label{sec:org691efb1}
\section{Organisational Methods}
\label{sec:orga8d02d5}
\section{Relevance to Degree}
\label{sec:org23a6533}
\section{Time Management}
\label{sec:org2efbd3d}
\section{Collaborative Work}
\label{sec:org29203b3}
\section{Other Issues}
\label{sec:orga813050}


\appendix
\chapter{\emph{Software Packages Used}}
\label{sec:org68bb70a}
\subsection{Libraries}
\label{sec:org421ae82}
\begin{table}[htbp]
\caption{\label{tab:org63dc111}
...
\subsection{Tools}
\label{sec:org13ba779}
\begin{table}[htbp]
\caption{\label{tab:org4fb746a}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:org051487c}
\begin{table}[htbp]
\caption{\label{tab:org1a013c6}
...
\label{sec:orgd8bc56e}
\begin{table}[htbp]
\caption{\label{tab:org0793ed6}
...
\label{sec:org9e6e746}

\section{Setup.py}
\label{sec:orgee7a412}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1]{python}
...
\label{sec:org88d4e57}
\label{org4345703}

\subsection{Sprint - Week 0}
\label{sec:org6c41471}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{orgc2e0939}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
