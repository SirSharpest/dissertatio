...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:org0456eb0}).

\section{Background}
\label{sec:org5636123}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:org28ab668}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org8a12913}
...
\label{sec:org178f683}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org6d8892e} and are as follows:
...
The individual images in figure:\ref{fig:org0973a16} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org082e6b2} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:orgf765b63}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org0973a16}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{org510ba9f}.

\section{Language Choices}
\label{sec:org030e8ff}
...
In figure:\ref{fig:orge4175f5} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:orge4175f5}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org7f57c19} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org7f57c19}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:orgaa2c301}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org4d9eeb7}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org41eb6e6}
...
Table:\ref{tab:orga921ff1} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org9d513a9}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{org2517358}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org2ba9999}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org9aa46bc}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:org9aa46bc}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:org94c829d}
...
\caption{\label{fig:orgb7d7c50}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org9dc5e3f}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:org9f44752}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:org9f44752}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org7d46744} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org7d46744}
...
\label{sec:org5ff9a9e}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:orga939099} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:orga939099}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org7d46744} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:org33d7b17}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:org33d7b17}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:orgb2278a9}
...
Here, the pipeline presented in figure:\ref{fig:orge094510} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orge094510}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:orge094510}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:orga47d865}
...
\label{sec:orga8551ac}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:org33d7b17}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org4416a44}.

\subsection{Implementation}
\label{sec:org3626126}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{org0533e15} this new algorithm, in full, is shown.
...
\label{sec:org103e611}

Figure:\ref{fig:orgf5397d6} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:orgf5397d6}
...
\label{sec:org7c826a1}

Figure:\ref{fig:orge4ab893} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:orge4ab893}
...
\end{minted}
\caption{\label{org7b9448b}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org2b4c56e}
...
\end{minted}
\caption{\label{org718079d}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:orgb1b509b}
...
\end{minted}
\caption{\label{org40f48b3}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:orgd86334f}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:org2130bca} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:org2130bca}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org425bebd} provides the python model implemented.
...
\caption{\label{fig:org9c63b6f}
The Initial Data Loading Window for the GUI}
\end{figure}
\subsubsection{Table View}
\label{sec:org4e33031}
...
\label{sec:org5715f5e}
In example:\ref{org68dd473} lines 11,12,13 show how the UI object has individual members, each of these has a "clicked" trigger, which when triggered calls a function pointer which this controller handles.
\begin{listing}[htbp]
...
One example of this is in the load data function shown in example:\ref{orgc45abc1}. Here the custom exception classes are imported and used for data already loaded, as well as the loading functions itself.
...
To do this, the existing canvas window is repainted based on parameter input. Example:\ref{org03b8e05}
...
An example of this is shown when dividing data by ploidy and comparing distributions via histograms in figure:\ref{fig:orgaf04f3e}.

\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/facet_example.png}
\caption{\label{fig:orgaf04f3e}
...
Example:\ref{org4800616} shows on lines 3,5,7 how decisions are made to best arrange the grid. Decisions are made based on the unique number of columns a grouping variable provides. As well this,  additional arguments are  used to indicate further aesthetics of the plots.
...
def facet_hist(self, df, column, group_by):
    ...
    return g.fig
\end{minted}
\caption{\label{org4800616}
Using Facet wrapping to provide}
\end{listing}

\section{Data Analysis Methods}
\label{sec:org5364c1a}
...
\label{sec:orgd437ebb}

Modern Bayesian interpretations use the simplified model of Bayes original theorem, equation:\ref{eq:org257fa25} is the basis of the implemented model in this project. This states that: the posterior is proportional to the likelihood times the prior.
...
\caption{\label{fig:orgd518ee7}
Normal\((\bar{x},2s)\)}
\end{figure}


\subsubsection{\(\sigma\) (Standard Deviation)}
\label{sec:org6a0fec6}
...
Figure:\ref{fig:orgbace908} provides the simulated data in the form of a histogram where two groups, 'g1' and 'g2' have been used. These are considered the input for the model priors.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/exampledata.png}
\caption{\label{fig:orgbace908}
Histogram of example input data}
\end{figure}

\subsubsection{Posterior Output}
\label{sec:org1cc7e05}
By applying the method outlined in code example:\ref{org425bebd} and defined by equation:\ref{eq:org7104864} the posterior predictions can generate 1000 random samples from the perceived distributions of the data. Based on the probability of each value existing within the priors' distribution.

Figure:\ref{fig:org587ee64} demonstrates the expanded data set created by the posterior predictions.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes.png}
\caption{\label{fig:org587ee64}
Histogram of example posterior data}
\end{figure}

\subsubsection{Difference of Means}
\label{sec:org9b89adf}
...
For the provided example data this model states that there is a 25.3\% chance that the two means of these groups are different. This is represented in figure:\ref{fig:org945ff66}.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes2.png}
\caption{\label{fig:org945ff66}
...
\label{sec:org2ecaec0}
\subsection{Length}
\label{sec:org6e94d37}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group1/length.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group1/bayes_difference_of_means_length.png}
    }
    \caption{Length of \textit{T. monococcum} and \textit{T. beoticum}, compared}
\end{figure}

\clearpage
\subsection{Width}
\label{sec:org54dd2c6}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group1/width.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group1/bayes_difference_of_means_width.png}
    }
    \caption{Width of \textit{T. monococcum} and \textit{T. beoticum}, compared}
\end{figure}

\clearpage
\subsection{Volume}
\label{sec:org14d4699}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group1/volume.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group1/bayes_difference_of_means_volume.png}
    }
    \caption{Volume of \textit{T. monococcum} and \textit{T. beoticum}, compared}
\end{figure}

\clearpage
\subsection{Geometric Ratio}
\label{sec:org78f7a29}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group1/length_depth_width.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group1/bayes_difference_of_means_length_depth_width.png}
    }
    \caption{Length Depth Width ratio of \textit{T. monococcum} and \textit{T. beoticum}, compared}
\end{figure}

\clearpage
\subsection{Pair Plot}
\label{sec:org8b17f54}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group1/pairplot.png}
\label{fig:org016edfc}
\end{figure}

\clearpage
\subsection{PCA}
\label{sec:org1d72bec}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group1/pca.png}
\label{fig:org8c729b7}
\end{figure}

\section{\emph{T. dicoccum} and \emph{T. dicoccoides}}
\label{sec:org5c27c27}

\subsection{Length}
\label{sec:org09aaff0}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group2/length.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group2/bayes_difference_of_means_length.png}
    }
    \caption{Length of \textit{T. dicoccum} and \textit{T. dicoccoides}, compared}
\end{figure}

\clearpage
\subsection{Width}
\label{sec:org4cda6e2}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group2/width.png}	\includegraphics[width=0.49\textwidth]{./images/results/group2/bayes_difference_of_means_width.png}
    }
    \caption{Width of \textit{T. dicoccum} and \textit{T. dicoccoides}, compared}
\end{figure}

\clearpage
\subsection{Volume}
\label{sec:orgc3dd11a}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group2/volume.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group2/bayes_difference_of_means_volume.png}
    }
    \caption{Volume of \textit{T. dicoccum} and \textit{T. dicoccoides}, compared}
\end{figure}

\clearpage
\subsection{Geometric Ratio}
\label{sec:orgefc4d26}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group2/length_depth_width.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group2/bayes_difference_of_means_length_depth_width.png}
    }
    \caption{Length Depth Width ratio of \textit{T. dicoccum} and \textit{T. dicoccoides}, compared}
\end{figure}

\clearpage
\subsection{Pair Plot}
\label{sec:org3fe7b49}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group2/pairplot.png}
\label{fig:orgea5c3c5}
\end{figure}

\clearpage
\subsection{PCA}
\label{sec:orga712887}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group2/pca.png}
\label{fig:orgf026afd}
\end{figure}



\section{\emph{T. spelta} and \emph{T. aestivum}}
\label{sec:org78f3b06}

\subsection{Length}
\label{sec:org0fb600b}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group3/length.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group3/bayes_difference_of_means_length.png}
    }
    \caption{Length of \textit{T. spelta} and \textit{T. aestivium}, compared}
\end{figure}

\clearpage
\subsection{Width}
\label{sec:org76a9e7f}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group3/width.png}	\includegraphics[width=0.49\textwidth]{./images/results/group3/bayes_difference_of_means_width.png}
    }
    \caption{Width of \textit{T. spelta} and \textit{T. aestivium}, compared}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group3/volume.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group3/bayes_difference_of_means_volume.png}
    }
    \caption{Volume of \textit{T. spelta} and \textit{T. aestivium}, compared}
\end{figure}

\clearpage
\subsection{Geometric Ratio}
\label{sec:org4758755}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group3/length_depth_width.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group3/bayes_difference_of_means_length_depth_width.png}
    }
    \caption{Length Depth Width ratio of \textit{T. spelta} and \textit{T. aestivium}, compared}
\end{figure}

\clearpage
\subsection{Pair Plot}
\label{sec:org7e3e300}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group3/pairplot.png}
\label{fig:orgd7cb54a}
\end{figure}

\clearpage
\subsection{PCA}
\label{sec:orga50944f}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group3/pca.png}
\label{fig:org0ff71ef}
\end{figure}
...
\label{sec:org698432d}

\subsection{Length}
\label{sec:org0205ae7}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group4/length.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group4/bayes_difference_of_means_length.png}
    }
    \caption{Length of \textit{T. beoticum} and \textit{T. dicoccoides}, compared}
\end{figure}

\clearpage
\subsection{Width}
\label{sec:org4e776cd}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group4/width.png}	\includegraphics[width=0.49\textwidth]{./images/results/group4/bayes_difference_of_means_width.png}
    }
    \caption{Width of \textit{T. beoticum} and \textit{T. dicoccoides}, compared}
\end{figure}

\clearpage
\subsection{Volume}
\label{sec:org51b2fb1}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group4/volume.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group4/bayes_difference_of_means_volume.png}
    }
    \caption{Volume of \textit{T. beoticum} and \textit{T. dicoccoides}, compared}
\end{figure}

\clearpage
\subsection{Geometric Ratio}
\label{sec:org12c1555}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group4/length_depth_width.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group4/bayes_difference_of_means_length_depth_width.png}
    }
    \caption{Length Depth Width ratio of \textit{T. beoticum} and \textit{T. dicoccoides}, compared}
\end{figure}

\clearpage
\subsection{Pair Plot}
\label{sec:orgc6de28e}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group4/pairplot.png}
\label{fig:org9f899d7}
\end{figure}

\clearpage
\subsection{PCA}
\label{sec:orgdf369ae}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group4/pca.png}
\label{fig:orgc1ae2d1}
\end{figure}
...
\label{sec:org4478f0b}

\subsection{Length}
\label{sec:org29dac72}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group5/length.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group5/bayes_difference_of_means_length.png}
    }
    \caption{Length of \textit{T. dicoccum} and \textit{T. durum}, compared}
\end{figure}

\clearpage
\subsection{Width}
\label{sec:org5bca711}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group5/width.png}	\includegraphics[width=0.49\textwidth]{./images/results/group5/bayes_difference_of_means_width.png}
    }
    \caption{Width of \textit{T. dicoccum} and \textit{T. durum}, compared}
\end{figure}

\clearpage
\subsection{Volume}
\label{sec:orge123de7}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group5/volume.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group5/bayes_difference_of_means_volume.png}
    }
    \caption{Volume of \textit{T. dicoccum} and \textit{T. durum}, compared}
\end{figure}

\clearpage
\subsection{Geometric Ratio}
\label{sec:org468b086}

\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.49\textwidth]{./images/results/group5/length_depth_width.png}
        \includegraphics[width=0.49\textwidth]{./images/results/group5/bayes_difference_of_means_length_depth_width.png}
    }
    \caption{Length Depth Width ratio of \textit{T. dicoccum} and \textit{T. durum}, compared}
\end{figure}

\clearpage
\subsection{Pair Plot}
\label{sec:org2a2ed0c}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group5/pairplot.png}
\label{fig:orgab1cd80}
\end{figure}

\clearpage
\subsection{PCA}
\label{sec:orgb47d868}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group5/pca.png}
\label{fig:org576d81f}
\end{figure}
...
\chapter{Discussion}
\label{sec:org9d14fb2}
\section{Similar Research}
\label{sec:org81adc80}
\section{Alternate Solutions}
\label{sec:org7848eea}
\chapter{Critical Evaluation}
\label{sec:orgc4c7a4b}
\section{Organisational Methods}
\label{sec:org1ede197}
\section{Relevance to Degree}
\label{sec:orge1175e1}
\section{Time Management}
\label{sec:org5d6ef27}
\section{Collaborative Work}
\label{sec:org8536a38}
\section{Other Issues}
\label{sec:orga1ce84f}


\appendix
\chapter{\emph{Software Packages Used}}
\label{sec:org8ad3fe7}
\subsection{Libraries}
\label{sec:orgc467528}
\begin{table}[htbp]
\caption{\label{tab:orga921ff1}
...
\subsection{Tools}
\label{sec:orgf8d9457}
\begin{table}[htbp]
\caption{\label{tab:org955b693}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:orgad26b57}
\begin{table}[htbp]
\caption{\label{tab:org0456eb0}
...
\label{sec:orgde30ab5}
\begin{table}[htbp]
\caption{\label{tab:org28ab668}
...
\label{sec:org5c0767b}

\section{Setup.py}
\label{sec:org132088b}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true,fontfamily=dejavu]{python}
...
\label{sec:org4764f9b}
\label{org510ba9f}

\subsection{Sprint - Week 0}
\label{sec:orga036819}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{orgb34a656}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
