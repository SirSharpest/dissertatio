...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:org4f958d1}).

\section{Background}
\label{sec:orgcea1688}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:orgc1358ca}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org3b0d9f2}
...
\label{sec:orgfbae04b}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:orgd6b1ffb} and are as follows:
...
The individual images in figure:\ref{fig:org547dde8} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:orge6a30af} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:orgd9c4bf5}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org547dde8}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{orgf9d3271}.

\section{Language Choices}
\label{sec:org4f44a63}
...
In figure:\ref{fig:org5d1fae8} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org5d1fae8}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org4eb115f} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org4eb115f}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:org144e5b2}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{orgfb5192c}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{orgaf6d62a}
...
Table:\ref{tab:org9523f44} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org70709ce}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{orgfb0e313}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org9a5a8a2}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org881cefa}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:org881cefa}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:orge665485}
...
\caption{\label{fig:org13dbc22}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org43b4e2b}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:orgfa8db15}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:orgfa8db15}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org25d2625} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org25d2625}
...
\label{sec:org17cc944}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:orge92acb8} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:orge92acb8}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org25d2625} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:orgbc8c930}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:orgbc8c930}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:orgfd7871e}
...
Here, the pipeline presented in figure:\ref{fig:org97fdc22} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:org97fdc22}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:org97fdc22}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:org782753c}
...
\label{sec:orgb6cb581}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:orgbc8c930}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org293a65b}.

\subsection{Implementation}
\label{sec:orgb27e77e}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{org2c65541} this new algorithm, in full, is shown.
...
\label{sec:orgacbed13}

Figure:\ref{fig:org81780c2} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:org81780c2}
...
\label{sec:org4112179}

Figure:\ref{fig:org332bb2a} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:org332bb2a}
...
\end{minted}
\caption{\label{orgc1d6005}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org04574d6}
...
\end{minted}
\caption{\label{org1a364c9}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:org464e3c9}
...
\end{minted}
\caption{\label{org021dde3}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:orgec6e1d1}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:org26b21f3} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:org26b21f3}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org6d0df8a} provides the python model implemented.
...
\caption{\label{fig:orgc50f9f9}
The Initial Data Loading Window for the GUI}
\end{figure}
\subsubsection{Table View}
\label{sec:orgeccfd5c}
...
\label{sec:org8472c11}
In example:\ref{org048f90e} lines 11,12,13 show how the UI object has individual members, each of these has a "clicked" trigger, which when triggered calls a function pointer which this controller handles.
\begin{listing}[htbp]
...
One example of this is in the load data function shown in example:\ref{orga788b50}. Here the custom exception classes are imported and used for data already loaded, as well as the loading functions itself.
...
\end{minted}
\caption{\label{orga788b50}
The load data function from the load\_data window}
\end{listing}
\subsection{Plot Window Rendering}
\label{sec:org7262d0d}
\subsection{Creating GUI Elements When Required}
\label{sec:orge325ca4}
\subsection{Dynamically Generating Plots}
\label{sec:orgadb61f8}
\section{Data Analysis Methods}
\label{sec:org1cb0877}
\section{Bayesian Modelling}
\label{sec:orgf32b753}


\chapter{Results}
\label{sec:org1438bbb}
\chapter{Discussion}
\label{sec:org6139a80}
\section{Similar Research}
\label{sec:org15b1d37}
\section{Alternate Solutions}
\label{sec:orgd5316ad}
\chapter{Critical Evaluation}
\label{sec:orgb82ff4d}
\section{Organisational Methods}
\label{sec:orgf946f9b}
\section{Relevance to Degree}
\label{sec:orge659fc5}
\section{Time Management}
\label{sec:org5fce14e}
\section{Collaborative Work}
\label{sec:org484c157}
\section{Other Issues}
\label{sec:org655460d}


\appendix
\chapter{\emph{Software Packages Used}}
\label{sec:org7b6ad44}
\subsection{Libraries}
\label{sec:orgd57a146}
\begin{table}[htbp]
\caption{\label{tab:org9523f44}
...
\subsection{Tools}
\label{sec:org5f8373c}
\begin{table}[htbp]
\caption{\label{tab:org93cc343}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:org021e874}
\begin{table}[htbp]
\caption{\label{tab:org4f958d1}
...
\label{sec:org0f76f2d}
\begin{table}[htbp]
\caption{\label{tab:orgc1358ca}
...
\label{sec:orgfecb0d4}

\section{Setup.py}
\label{sec:orgb22c1ac}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true,fontfamily=dejavu]{python}
...
\label{sec:org858fef2}
\label{orgf9d3271}

\subsection{Sprint - Week 0}
\label{sec:org110c2cd}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{orge3f90bc}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
