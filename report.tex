...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:orgfaad0ef}).

\section{Background}
\label{sec:orga9fb30c}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:org2a82087}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org7a5c8c4}
...
\label{sec:orgc01f2bd}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:orgb3c71d3} and are as follows:
...
The individual images in figure:\ref{fig:orgebce89e} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org2a100c7} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:orgfdd6eb9}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:orgebce89e}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{org89a8397}.

\section{Language Choices}
\label{sec:org70cc24e}
...
In figure:\ref{fig:org248f43c} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org248f43c}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org0636764} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org0636764}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:org8eced2c}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{orgeb63258}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{orgc0390fd}
...
Table:\ref{tab:org68b697d} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org13cf3dc}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{org77f675a}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org6bb6033}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org2681d67}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:org2681d67}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:orgd630e5d}
...
\caption{\label{fig:orge7bf41d}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:orgc5915a3}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:org71b14d5}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:org71b14d5}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org86dbecb} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org86dbecb}
...
\label{sec:org71a50e6}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:orgec524af} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:orgec524af}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org86dbecb} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:orgc9e1ae1}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:orgc9e1ae1}
Image Processing Pipeline}
\end{figure}

\subsection{Morphometric Features}
\label{sec:org3edde0c}
...
Values used in statistical functions and measurements are presented as metric units, derived from \textmu{}-CT image pixel values. The equation:\ref{eq:orgba26aa8} is presented here.
...
Here, the pipeline presented in figure:\ref{fig:orge620113} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
An additional phenotype is created to describe the interaction between the geometric parameters; the interaction is described in equation:\ref{eq:org8f7c639}.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orge620113}.
\clearpage
\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/pipeline.png}
\caption{\label{fig:orge620113}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:orgaba9284}
...
\label{sec:org0cae804}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:orgc9e1ae1}). This provided much better results than the previous
...
\label{sec:orgcb9e1a1}

Figure:\ref{fig:org2f2fd4a} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:org2f2fd4a}
...
\label{sec:orgf305cf7}

Figure:\ref{fig:org7339d44} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/watershedfigure.png}
\caption{\label{fig:org7339d44}
...
\end{minted}
\caption{\label{org795b80e}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:orgea20b3b}
...
\end{minted}
\caption{\label{org8957ca4}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:orgd239b4a}
...
\end{minted}
\caption{\label{org5372aaa}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:orgfbd9ad5}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:org604cef4} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:org604cef4}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org4ae8b9f} provides the python model implemented.
...
\caption{\label{fig:org02693db}
The Initial Data Loading Window for the GUI}
\end{figure}
\subsubsection{Table View}
\label{sec:orgf999a5a}
...
\label{sec:orgec360d6}
In example:\ref{org44df65b} lines 11,12,13 show how the UI object has individual members, each of these has a "clicked" trigger, which when triggered calls a function pointer which this controller handles.
\begin{listing}[htbp]
...
One example of this is in the load data function shown in example:\ref{org962a280}. Here the custom exception classes are imported and used for data already loaded, as well as the loading functions itself.
...
To do this, the existing canvas window is repainted based on parameter input. Example:\ref{org632453b}
...
An example of this is shown when dividing data by ploidy and comparing distributions via histograms in figure:\ref{fig:org6aac8cb}.

\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/facet_example.png}
\caption{\label{fig:org6aac8cb}
...
Example:\ref{org2ff8a4a} shows on lines 3,5,7 how decisions are made to best arrange the grid. Decisions are made based on the unique number of columns a grouping variable provides. As well this,  additional arguments are  used to indicate further aesthetics of the plots.
...
def facet_hist(self, df, column, group_by):
    ...
    return g.fig
\end{minted}
\caption{\label{org2ff8a4a}
Using Facet wrapping to provide}
\end{listing}

\section{Data Analysis Methods}
\label{sec:orgaff3c84}
...
\label{sec:org5de87f2}

Modern Bayesian interpretations use the simplified model of Bayes original theorem, equation:\ref{eq:org6f4341b} is the basis of the implemented model in this project. This states that: the posterior is proportional to the likelihood times the prior.
...
\caption{\label{fig:org5723225}
Normal\((\bar{x},2s)\)}
\end{figure}


\subsubsection{\(\sigma\) (Standard Deviation)}
\label{sec:org3c9e0f6}
...
Figure:\ref{fig:org8ed9a33} provides the simulated data in the form of a histogram where two groups, 'g1' and 'g2' have been used. These are considered the input for the model priors.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/exampledata.png}
\caption{\label{fig:org8ed9a33}
Histogram of example input data}
\end{figure}

\subsubsection{Posterior Output}
\label{sec:orgd3fc9e0}
By applying the method outlined in code example:\ref{org4ae8b9f} and defined by equation:\ref{eq:org4549659} the posterior predictions can generate 1000 random samples from the perceived distributions of the data. Based on the probability of each value existing within the priors' distribution.

Figure:\ref{fig:org033294b} demonstrates the expanded data set created by the posterior predictions.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes.png}
\caption{\label{fig:org033294b}
Histogram of example posterior data}
\end{figure}

\subsubsection{Difference of Means}
\label{sec:org69fa088}
...
For the provided example data this model states that there is a 25.3\% chance that the two means of these groups are different. This is represented in figure:\ref{fig:orgbe07b13}.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes2.png}
\caption{\label{fig:orgbe07b13}
...
\clearpage
\section{Grain Length is Not Effected by Domestication}
\label{sec:org3025a1a}
...
\section{Grain Width Increases with Domestication}
\label{sec:org069df6c}

\section{Grain Volume Increases with Domestication}
\label{sec:org06de20c}

\section{Grain Volume Increases with Ploidy}
\label{sec:orga7fc12a}

\section{Hexaploid Genotypes Contain No Significant Variation}
\label{sec:org156cace}
...
\subsection{Tools}
\label{sec:org394b261}
\begin{table}[htbp]
\caption{\label{tab:org3f5246e}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\clearpage
\chapter{Glossary}
\label{sec:org3e3bf1f}
\begin{table}[htbp]
\caption{\label{tab:orgfaad0ef}
...
\label{sec:org5151ff6}
\begin{table}[htbp]
\caption{\label{tab:org2a82087}
...
\label{sec:org85945f6}

\subsection{Data Distributions}
\label{sec:org16f83c1}
...
\label{sec:orga933522}
\label{org89a8397}

\subsection{Sprint - Week 0}
\label{sec:orgd4b0842}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{org9542739}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
