...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:org9488235}).

\section{Background}
\label{sec:orge4e2ff2}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:orgbd40719}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org8e97756}
...
\label{sec:org0bb2163}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:org36a4603} and are as follows:
...
The individual images in figure:\ref{fig:orgc1b44f4} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:orge5c9855} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org1b501d2}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:orgc1b44f4}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{orgd3fbadc}.

\section{Language Choices}
\label{sec:org67ff663}
...
In figure:\ref{fig:org0fb3f6d} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org0fb3f6d}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org7864cff} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org7864cff}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:org50ff962}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{orge663f4c}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{org1e19667}
...
Table:\ref{tab:org132fec0} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org1cf16fc}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{org495808f}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org80ca654}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:orgd995fd0}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:orgd995fd0}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:org9ef1fac}
...
\caption{\label{fig:orgf282411}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org18dea82}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:orgc917a84}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:orgc917a84}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org20ca356} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org20ca356}
...
\label{sec:orgcce9ea5}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org6e28af7} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org6e28af7}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org20ca356} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:orgc1cf90a}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:orgc1cf90a}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org72abbdb}
...
Here, the pipeline presented in figure:\ref{fig:orga36d538} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orga36d538}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:orga36d538}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:org424c71f}
...
\label{sec:org0384f93}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:orgc1cf90a}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org39e9345}.

\subsection{Implementation}
\label{sec:orgf9b9977}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{org905d56c} this new algorithm, in full, is shown.
...
\label{sec:org885c311}

Figure:\ref{fig:orgc2c45d9} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:orgc2c45d9}
...
\label{sec:orgd7a388a}

Figure:\ref{fig:orgb1caae8} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:orgb1caae8}
...
\end{minted}
\caption{\label{orgc2c4721}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org8c87b9e}
...
\end{minted}
\caption{\label{orgaa18dbe}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:org075e413}
...
\end{minted}
\caption{\label{org1a00d17}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:org28ec512}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:org37aae49} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:org37aae49}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org67946f3} provides the python model implemented.
...
\caption{\label{fig:orgf859bbb}
The Initial Data Loading Window for the GUI}
\end{figure}
\subsubsection{Table View}
\label{sec:orge6b5e54}
...
\label{sec:org198ddc9}
In example:\ref{org950e84e} lines 11,12,13 show how the UI object has individual members, each of these has a "clicked" trigger, which when triggered calls a function pointer which this controller handles.
\begin{listing}[htbp]
...
One example of this is in the load data function shown in example:\ref{orgea1e7cb}. Here the custom exception classes are imported and used for data already loaded, as well as the loading functions itself.
...
To do this, the existing canvas window is repainted based on parameter input. Example:\ref{org0f6c6df}
...
An example of this is shown when dividing data by ploidy and comparing distributions via histograms in figure:\ref{fig:org99b3d01}.

\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/facet_example.png}
\caption{\label{fig:org99b3d01}
...
Example:\ref{org2a1a12f} shows on lines 3,5,7 how decisions are made to best arrange the grid. Decisions are made based on the unique number of columns a grouping variable provides. As well this,  additional arguments are  used to indicate further aesthetics of the plots.
...
def facet_hist(self, df, column, group_by):
    ...
    return g.fig
\end{minted}
\caption{\label{org2a1a12f}
Using Facet wrapping to provide}
\end{listing}

\section{Data Analysis Methods}
\label{sec:org2849daa}
...
\label{sec:org0b43bb2}

Modern Bayesian interpretations use the simplified model of Bayes original theorem, equation:\ref{eq:orgbd80e32} is the basis of the implemented model in this project. This states that: the posterior is proportional to the likelihood times the prior.
...
\caption{\label{fig:org962eee8}
Normal\((\bar{x},2s)\)}
\end{figure}


\subsubsection{\(\sigma\) (Standard Deviation)}
\label{sec:orga3e286c}
...
Figure:\ref{fig:org84bf69e} provides the simulated data in the form of a histogram where two groups, 'g1' and 'g2' have been used. These are considered the input for the model priors.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/exampledata.png}
\caption{\label{fig:org84bf69e}
Histogram of example input data}
\end{figure}

\subsubsection{Posterior Output}
\label{sec:orge350260}
By applying the method outlined in code example:\ref{org67946f3} and defined by equation:\ref{eq:orge4ff506} the posterior predictions can generate 1000 random samples from the perceived distributions of the data. Based on the probability of each value existing within the priors' distribution.

Figure:\ref{fig:org6cf76e9} demonstrates the expanded data set created by the posterior predictions.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes.png}
\caption{\label{fig:org6cf76e9}
Histogram of example posterior data}
\end{figure}

\subsubsection{Difference of Means}
\label{sec:org7f84698}
...
For the provided example data this model states that there is a 25.3\% chance that the two means of these groups are different. This is represented in figure:\ref{fig:org66720a1}.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes2.png}
\caption{\label{fig:org66720a1}
...
\chapter{Results}
\label{sec:org9ffb149}
\chapter{Discussion}
\label{sec:orgdb1eb4d}
\section{Similar Research}
\label{sec:orgf257e44}
\section{Alternate Solutions}
\label{sec:orgb40f5b2}
\chapter{Critical Evaluation}
\label{sec:orga38ad38}
\section{Organisational Methods}
\label{sec:org921bff9}
\section{Relevance to Degree}
\label{sec:orgff14dc8}
\section{Time Management}
\label{sec:orgcc68d25}
\section{Collaborative Work}
\label{sec:org91fe6ed}
\section{Other Issues}
\label{sec:org4eeaecc}


\appendix
\chapter{\emph{Software Packages Used}}
\label{sec:org48afcac}
\subsection{Libraries}
\label{sec:org2ee7785}
\begin{table}[htbp]
\caption{\label{tab:org132fec0}
...
\subsection{Tools}
\label{sec:org8f3d611}
\begin{table}[htbp]
\caption{\label{tab:org2cb4b02}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:org7fb4761}
\begin{table}[htbp]
\caption{\label{tab:org9488235}
...
\label{sec:org63da5fd}
\begin{table}[htbp]
\caption{\label{tab:orgbd40719}
...
\label{sec:orgb1fdfef}

\section{Setup.py}
\label{sec:orgf1b8127}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true,fontfamily=dejavu]{python}
...
\label{sec:orgf2512c0}
\label{orgd3fbadc}

\subsection{Sprint - Week 0}
\label{sec:orgf99d272}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{orga3459a7}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
