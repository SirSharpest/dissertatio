...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the \emph{glossary} (table:\ref{tab:orgcee1616}).

\section{Background}
\label{sec:org6b2f014}
2
...
\end{multicols}
Full species names are found in table:\ref{tab:org5bdea35}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:orgce516a1}
...
\label{sec:org78d6db1}

These samples come from over 70 plants and provided in excess of 2000 seeds for analysis which data was created based on. The traits recorded are labelled in figure:\ref{fig:orge036bf4} and are as follows:
...
The individual images in figure:\ref{fig:orga29c12a} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org215ef72} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org6aead6c}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:orga29c12a}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{orgf5ef155}.

\section{Language Choices}
\label{sec:org35260db}
...
In figure:\ref{fig:orgc04132f} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:orgc04132f}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org184edba} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org184edba}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:orgea868ca}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org2d39683}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{orge884cc8}
...
Table:\ref{tab:org4117e25} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:orgf03bc64}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{orgd87cf86}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org9d9d945}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:orgf8ccf9e}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:orgf8ccf9e}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:orgb0ec2ef}
...
\caption{\label{fig:org66261d6}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org615ea14}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:orgfc3534d}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:orgfc3534d}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org6d430e5} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org6d430e5}
...
\label{sec:orga84aa4a}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:org4f3d255} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:org4f3d255}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org6d430e5} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:orgf578215}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:orgf578215}
Image Processing Pipeline}
\end{figure}

\subsection{Data Analysis Pipeline}
\label{sec:org27b27fc}
...
Here, the pipeline presented in figure:\ref{fig:org86fd73a} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:org86fd73a}.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/pipeline.png}
\caption{\label{fig:org86fd73a}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:orgb978f36}
...
\label{sec:orgb9fb70e}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:orgf578215}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:org3817e57}.

\subsection{Implementation}
\label{sec:orga5c6391}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{orgf8245c3} this new algorithm, in full, is shown.

\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true]{octave}
...
\label{sec:orgdaa2a70}

Figure:\ref{fig:orgde1b937} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:orgde1b937}
...
\label{sec:org84e8061}

Figure:\ref{fig:orgdad1e13} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./images/watershedfigure.png}
\caption{\label{fig:orgdad1e13}
...
\end{minted}
\caption{\label{orgec06a1b}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org92ec99f}
...
\end{minted}
\caption{\label{org0ba2914}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:orge77ed92}
...
\end{minted}
\caption{\label{orga202fcf}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:orgd90e6b5}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:org6db9df8} provides an example of such.

\#+ATTR\_LATEX :width 10cm
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./images/ci.png}
\caption{\label{fig:org6db9df8}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{orgdaf754f} provides the python model implemented.
...
\label{sec:org620be16}
\subsection{Connecting Signals from GUI to Function Calls}
\label{sec:org425f577}
\subsection{Plot Window Rendering}
\label{sec:orgb6e1bdc}
\subsection{Creating GUI Elements When Required}
\label{sec:org34bb180}
\subsection{Dynamically Generating Plots}
\label{sec:orgd6fd6cd}
\section{Data Analysis Methods}
\label{sec:orge96a7fb}
\section{Bayesian Modelling}
\label{sec:orgc119e04}

\chapter{Results}
\label{sec:org6b3daa0}
\chapter{Discussion}
\label{sec:org5202b86}
\section{Similar Research}
\label{sec:org61da20a}
\section{Alternate Solutions}
\label{sec:org10de7e9}
\chapter{Critical Evaluation}
\label{sec:org3f32267}
\section{Organisational Methods}
\label{sec:org78241db}
\section{Relevance to Degree}
\label{sec:orgcdb23a7}
\section{Time Management}
\label{sec:orgd3f03ac}
\section{Collaborative Work}
\label{sec:orgb2209a3}
\section{Other Issues}
\label{sec:org199df31}


\appendix
\chapter{\emph{Software Packages Used}}
\label{sec:org60b9910}
\subsection{Libraries}
\label{sec:org0bebe9b}
\begin{table}[htbp]
\caption{\label{tab:org4117e25}
...
\subsection{Tools}
\label{sec:org747e406}
\begin{table}[htbp]
\caption{\label{tab:orgab89e02}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\label{sec:org4cb8e32}
\begin{table}[htbp]
\caption{\label{tab:orgcee1616}
...
\label{sec:orgfcde7cc}
\begin{table}[htbp]
\caption{\label{tab:org5bdea35}
...
\label{sec:org45eca92}

\section{Setup.py}
\label{sec:orgbd7bbf4}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true]{python}
...
\label{sec:orgefef691}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true]{common-lisp}
...
\label{sec:orgf2524f6}
\label{orgf5ef155}

\subsection{Sprint - Week 0}
\label{sec:org75f9fa0}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{org1cae0b7}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
