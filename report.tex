...
Additionally, as this is very much multi-disciplinary research, specific terms and definitions have been outlined in the glossary (table:\ref{tab:org6c11fff}).
...
\end{multicols}
Full species names are found in table:\ref{tab:org146ff0a}.

\subsection{Why use \textmu{}-CT image analysis?}
\label{sec:org26e5b85}
...
\label{sec:orgaa4c573}

These samples come from over 70 plants and provided in excess of 2000 seeds. These data form the basis for this research and testing of the software libraries created. The traits recorded are labelled in figure:\ref{fig:orgc39abbf} and are as follows:
...
The individual images in figure:\ref{fig:org146dffd} show, at a glance, the diversity and also the difference in the wild and cultivated (domesticated)
...
Novel additions have been made to existing image analysis libraries in order to make them more flexible for this project. Figure:\ref{fig:org7181705} illustrates the range of diversity
...
\section{Hypothesis}
\label{sec:org3f390e2}
To provide a full spectrum of analysis the null-hypothesis of this work is presented as investigating if there are morphometric differences in the seeds of several wheat varieties outlined in figure:\ref{fig:org146dffd}.
...
A full account of the sprints, which are in part running documentation for this project, are available in appendix:\ref{orge934591}.

\section{Language Choices}
\label{sec:org0a7c8d3}
...
In figure:\ref{fig:org39f1445} an example of the wire-frames created during meetings is show (A), next to it is displayed the final look of the loading window (B).
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe1.png}
\caption{\label{fig:org39f1445}
Wire-frame of the GUI loading data window}
\end{figure}

Similarly, figure:\ref{fig:org394d752} provides the initial wire-frame (A) of how the analysis window could have looked and what kind of GUI elements would be required, again, next to it is the final analysis window (B)
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/wireframe2.png}
\caption{\label{fig:org394d752}
Wire-frame of the GUI analysis window}
\end{figure}
\section{Documentation}
\label{sec:orga787b3f}
...
This documentation generator was purpose created, implemented in LISP and provided in listing:\ref{org430e524}.

Beyond this, inline commenting is provided for supplied software. Keeping in line with the agile development ethos the software is self-documented and self-evident. A brief example of this is shown in listing:\ref{orgaad434c}
...
Table:\ref{tab:org4dc1d5f} contains a full listing of all software used and required by this project.
\subsection{Numpy}
\label{sec:org74d3346}
...
Additionally, \emph{setup.py} has been used to provide installation of the library, the code for this can be seen in listing:\ref{org418ad74}. Using \emph{setup.py} provides a quick and easy way for any user to install the software, along with any dependencies.

\subsubsection{Issue Tracking}
\label{sec:org2bb757a}
Issues were tracked during the project, both in personal notes and in the Git interface as illustrated in figure:\ref{fig:org6552e2f}
\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/github.png}
\caption{\label{fig:org6552e2f}
Github Issue Tracking}
\end{figure}

\section{Implementation Methodology}
\label{sec:orgd12466b}
...
\caption{\label{fig:org90a8a16}
CT Analysing GUI UML}
\end{figure}
\section{Library Versioning}
\label{sec:org8d42529}
...
These provided a method of acceptance testing by those who would be using the software to help with investigating data. Page 1 of the given form is shown in figure:\ref{fig:org2c1cb11}. This form was completed by 3 researchers at the National Plant Phenomics Centre, feedback was very positive overall.
\begin{figure}[htbp]
\centering
\includegraphics[width=7.3cm]{./images/feedbackform.png}
\caption{\label{fig:org2c1cb11}
CT Feedback form}
\end{figure}
...
In table:\ref{tab:org26ad8f8} the results of the automated testing is given along side an image of several of the tests, tests of the same graphs but with different parameters were also generated and manually verified and provided as supplemental data.

\begin{longtable}{|l|l|p{4.4cm}|C|}
\caption{\label{tab:org26ad8f8}
...
\label{sec:org2008ac6}
These unit tests, alongside user tests have been used to meet the outlined functional requirements; specific tests are shown in table:\ref{tab:orge80ecb8} to match them to the associated functional requirement.

\begin{longtable}{|l|l|p{14cm}|}
\caption{\label{tab:orge80ecb8}
...
\hline
16 & 21-27 & Several tests, those shown in table:\ref{tab:org26ad8f8} are specifically recorded visually and presented as images in order to automatically unit test that visual data is intact\\
\hline
...
The entire process is represented as a diagram in figure:\ref{fig:org51b15b3}

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/matlab.png}
\caption{\label{fig:org51b15b3}
Image Processing Pipeline}
\end{figure}

\subsection{Morphometric Features}
\label{sec:org367b9b8}
...
Values used in statistical functions and measurements are presented as metric units, derived from \textmu{}-CT image pixel values. The equation:\ref{eq:org3df9055} is presented here.
...
Here, the pipeline presented in figure:\ref{fig:orgdc4f3ca} demonstrates the process used by the CT Analysis Library, and by result the CT Analysing GUI.
...
An additional phenotype is created to describe the interaction between the geometric parameters; the interaction is described in equation:\ref{eq:org4d65bef}.
...
From statistical testing a final result(s) can be used to answer scientific questions on the data. The entire process is represented in figure:\ref{fig:orgdc4f3ca}.
\clearpage
\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/pipeline.png}
\caption{\label{fig:orgdc4f3ca}
How data is integrated with the CT Analysing Library}
\end{figure}

\section{Image Analysis Methods}
\label{sec:orgc59cbc6}
...
\label{sec:org8edc659}

In order to solve the problem of misidentified and joint seeds, from the primitive collection,
a  \emph{quasi-euclidean} distance transform was implemented into the analysis pipeline (figure:\ref{fig:org51b15b3}). This provided much better results than the previous
...
and enabling fast distance transforms via Rosenfeld and Pfaltz's \emph{quasi-euclidean} method stated in equation:\ref{eq:orgfb3651f}.

\subsection{Implementation}
\label{sec:orgb551a27}

The software implementation for this was done in MATLAB \cite{MATHWORKS2017}, this meant that it could instantly and easily be inserted into the existing pipeline in the open source software which was forked and modified to add this feature. In code listing:\ref{orgfae79be} this new algorithm, in full, is shown.
...
\label{sec:orgabbbee8}

Figure:\ref{fig:org57ccf80} shows in two dimensional flat images how this problem was solved. As well as what the previous method was doing to cause errors in data.
\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{./images/chess_quasi.png}
\caption{\label{fig:org57ccf80}
...
\label{sec:orge07f401}

Figure:\ref{fig:orgeb3314c} shows the effects on grains in three dimensions, where the seeds which where incorrectly split have now been vastly improved, leading to an increase in result accuracy.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/watershedfigure.png}
\caption{\label{fig:orgeb3314c}
...
\end{minted}
\caption{\label{orgd83d62e}
CT Data Object Initialisation Method}
\end{listing}

\clearpage
\subsection{Loading Data}
\label{sec:org81fc3cd}
...
\end{minted}
\caption{\label{orgb534904}
Cleaning Data}
\end{listing}

\clearpage
\subsection{Data Transformations (PCA)}
\label{sec:orgb863026}
...
\end{minted}
\caption{\label{org114d17e}
PCA Data Transform}
\end{listing}

\clearpage
\subsection{Bayesian Hypothesis Testing}
\label{sec:org15a8a0d}
...
This function returns a \emph{trace} object which contains all of the data generated, the data of particular interest is the "difference of means" that this method calculates and the credible interval graphs which it returns the values for, figure:\ref{fig:orgd4d6a15} provides an example of such. The example provided shows the likelihood of two means of an interaction term of
\(length \times  depth \times width\) overlapping.

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/ci.png}
\caption{\label{fig:orgd4d6a15}
An example credible interval produced by this method}
\end{figure}


The full model is explained later in this chapter in a much more informative manner and fully defined in terms of relevance to results of this project. Listing:\ref{org3aa3629} provides the python model implemented.
...
\caption{\label{fig:org292e917}
The Initial Data Loading Window for the GUI}
\end{figure}
\subsubsection{Table View}
\label{sec:org9ad5fec}
...
\label{sec:orgbcc2a2f}
In example:\ref{org9310af4} lines 11,12,13 show how the UI object has individual members, each of these has a "clicked" trigger, which when triggered calls a function pointer which this controller handles.
\begin{listing}[htbp]
...
One example of this is in the load data function shown in example:\ref{org65199dc}. Here the custom exception classes are imported and used for data already loaded, as well as the loading functions itself.
...
To do this, the existing canvas window is repainted based on parameter input. Example:\ref{org094b200}
...
An example of this is shown when dividing data by ploidy and comparing distributions via histograms in figure:\ref{fig:org39289b3}.

\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./images/facet_example.png}
\caption{\label{fig:org39289b3}
...
Example:\ref{org4b45982} shows on lines 3,5,7 how decisions are made to best arrange the grid. Decisions are made based on the unique number of columns a grouping variable provides. As well this,  additional arguments are  used to indicate further aesthetics of the plots.
...
def facet_hist(self, df, column, group_by):
    ...
    return g.fig
\end{minted}
\caption{\label{org4b45982}
Using Facet wrapping to provide}
\end{listing}

\section{Data Analysis Methods}
\label{sec:org6e62f92}
...
\label{sec:orge2ef234}

Modern Bayesian interpretations use the simplified model of Bayes original theorem, equation:\ref{eq:org753bb03} is the basis of the implemented model in this project. This states that: the posterior is proportional to the likelihood times the prior.
...
\caption{\label{fig:orgfa3bd13}
Normal\((\bar{x},2s)\)}
\end{figure}


\subsubsection{\(\sigma\) (Standard Deviation)}
\label{sec:org7643c3d}
...
Figure:\ref{fig:orgf6b50d1} provides the simulated data in the form of a histogram where two groups, 'g1' and 'g2' have been used. These are considered the input for the model priors.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/exampledata.png}
\caption{\label{fig:orgf6b50d1}
Histogram of example input data}
\end{figure}

\subsubsection{Posterior Output}
\label{sec:org357a099}
By applying the method outlined in code example:\ref{org3aa3629} and defined by equation:\ref{eq:org86679a8} the posterior predictions can generate 1000 random samples from the perceived distributions of the data. Based on the probability of each value existing within the priors' distribution.

Figure:\ref{fig:orgfed5e0c} demonstrates the expanded data set created by the posterior predictions.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes.png}
\caption{\label{fig:orgfed5e0c}
Histogram of example posterior data}
\end{figure}

\subsubsection{Difference of Means}
\label{sec:org1693016}
...
For the provided example data this model states that there is a 25.3\% chance that the two means of these groups are different. This is represented in figure:\ref{fig:org245ac59}.
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./images/examplebayes2.png}
\caption{\label{fig:org245ac59}
...
Hexaploids (\emph{T. aestivum}) when compared to tetraploids (\emph{T. dicoccum}) reports a 86.2\% chance of the two means being the same. Indicating that increasing of ploidy fron 4N to 6N does not produce a significant change in grain volume. Figure:\ref{fig:orgb408e14} shows this, with full results shown in supplemental figure:\ref{fig:group6}.

\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./images/results/ploidyvol.png}
\caption{\label{fig:orgb408e14}
...
A wide range of attributes which previous studies could not access \cite{Gegas2010} is made available for study. In figure:\ref{fig:orga6919ec} the available phenotypes are displayed in a pairwise fashion, demonstrating how interesting relationships between attributes could provide more information if studied further.
...
With this data, similar solutions could be implemented. Figure:\ref{fig:org48510c1} shows all grains in this study, grouped by origin spike and plotted by a two component PCA. Input is calculated through averaging across spike grains' length, width, depth, volume and the geometric interaction term, via equation:\ref{eq:orgf65e128}.
...
\label{sec:org54fde5f}
\section{Organisational Methods}
\label{sec:orgb163bbe}
\section{Relevance to Degree}
\label{sec:org0aa6f76}
\section{Time Management}
\label{sec:orgfbe2b9a}
\section{Collaborative Work}
\label{sec:org2503618}
\section{Other Issues}
\label{sec:org2ce7344}


\appendix

\chapter{Glossary}
\label{sec:org0b2cf41}
\begin{table}[htbp]
\caption{\label{tab:org6c11fff}
...
\label{sec:org03a14c8}

\subsection{Libraries}
\label{sec:org40c3599}
\begin{table}[htbp]
\caption{\label{tab:org4dc1d5f}
...
\subsection{Tools}
\label{sec:orgbe66d7e}
\begin{table}[htbp]
\caption{\label{tab:org53d6b83}
Software tools used}
\centering
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
...
\section{Documentation}
\label{sec:orgb68fef0}
\clearpage

\chapter{Wheat Varieties}
\label{sec:orgfe5151b}
\begin{table}[htbp]
\caption{\label{tab:org146ff0a}
...
\label{sec:org14a7d7e}

\section{Setup.py}
\label{sec:orgca3f84a}
\begin{listing}[htbp]
\begin{minted}[linenos,firstnumber=1,frame=lines,linenos=true,fontfamily=DejaVuSans]{python}
...
\subsection{Pair Plot}
\label{sec:org03ccda8}

\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group2/pairplot.png}
\label{fig:orgef856f5}
\end{figure}

\clearpage
\subsection{PCA}
\label{sec:org05cfbee}
\begin{figure}[htbp]
\centering
\includegraphics[width=18cm]{./images/results/group2/pca.png}
\label{fig:org039cb25}
\end{figure}
...
\label{sec:orgeb8cd3b}

\subsection{Data Distributions}
\label{sec:org5e901e0}
...
\label{sec:org0ec74f2}

\subsection{Data Distributions}
\label{sec:orgde62c77}
...
A key function of the CT Analysis Library was created, showing in listing:\ref{orgb72cf27}. The method enables additional experiment information to be joined with extracted seed data. This provides a way of grouping seeds into more useful groupings than just their scanning data.
